...
<ul>
	...
	<li>Параметры, на которые влияет:
		<ul>
			<li><strong>tree_parent_id</strong> - ID родительской ноды</li>
	...
	<li><strong>isdata_N</strong> - где <strong>N</strong> - ID дата поля (<a href="/admin/types_data/1">все поля</a>). Обновляется, если есть obj_id или ведущее поле isdata_N. Обновляет поля объекта и его DATA-поля.</li>
</ul>
...
<p>Если требуется обновить поля по определенному DATA-полю, то следует взять его ID и поставить его первым столбцом в файле с ключом <em>isdata_ID</em>, где ID - id DATA-поля.</p>

<p>Для того, чтобы пропустить обновление конкретного поля у конкретного объекта, поставьте для него значение - <strong>_NOTSAVE_</strong></p>
